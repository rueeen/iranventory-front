...
export function ProtectedRoute({ children, roles }: ProtectedRouteProps) {
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#111827] px-6 text-white">
        <div className="flex flex-col items-center gap-4">
          <div className="h-10 w-10 animate-spin rounded-full border-4 border-red-300 border-t-transparent" />
  ...
  if (roles && !tieneRol(usuario, roles)) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center gap-3 bg-[#F3F4F6] px-6 text-center text-slate-900">
        <p className={`text-sm font-semibold uppercase tracking-widest ${clasesInacap.textoMarca}`}>403</p>
